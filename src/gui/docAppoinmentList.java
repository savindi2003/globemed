 ...
 * @author Savindi
 */
public class docAppoinmentList extends javax.swing.JFrame {
...
private User currentUser;
    ...
    public docAppoinmentList(User currentUser, PermissionService permissionService) {
        initComponents();
    ...
    private void loadTable() {
        try {
            ...
            ResultSet rs = MySQL.execute("SELECT  * FROM `appoinment` "
                ...
                + "INNER JOIN `room` ON `room`.`id` = `appoinment`.`room_id` WHERE `doctor_app`.`doc_id` = '"+user_id+"' ");
            ...
            while (rs.next()) {
                ...
                v.add(safeDecrypt(rs.getString("patient.name")));
        ...
        try {
            ...
            ResultSet rs = MySQL.execute("SELECT  * FROM `appoinment` "
                ...
                + "INNER JOIN `room` ON `room`.`id` = `appoinment`.`room_id` "
                    + "WHERE `appoinment`.`appoinment_id` LIKE '%" + query + "%' AND `doctor_app`.`doc_id` = '"+user_id+"' ");
    ...
    private void jTextField2KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextField2KeyReleased
        try {
            ...
            ResultSet rs = MySQL.execute("SELECT  * FROM `appoinment` "
                ...
                + "INNER JOIN `patient` ON `patient`.`id` = `appoinment`.`patient_id` "
                + "INNER JOIN `room` ON `room`.`id` = `appoinment`.`room_id` "
                    + "WHERE `patient`.`name` LIKE '%" + query + "%' AND `doctor_app`.`doc_id` = '"+user_id+"' ");
    ...
    private void jComboBox1ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jComboBox1ItemStateChanged
        try {
            ...
            ResultSet rs =MySQL.execute("SELECT  * FROM `appoinment` "
                ...
                + "INNER JOIN `patient` ON `patient`.`id` = `appoinment`.`patient_id` "
                + "INNER JOIN `room` ON `room`.`id` = `appoinment`.`room_id` "
                    + "WHERE `appoinment`.`reason_id` = '" + query + "' AND `doctor_app`.`doc_id` = '"+user_id+"' ");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            ResultSet rs = MySQL.execute("SELECT  * FROM `appoinment` "
                ...
                + "INNER JOIN `patient` ON `patient`.`id` = `appoinment`.`patient_id` "
                + "INNER JOIN `room` ON `room`.`id` = `appoinment`.`room_id` "
                    + "WHERE `appoinment`.`date` = '" + query + "' AND `doctor_app`.`doc_id` = '"+user_id+"' ");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new docAppoinmentList().setVisible(true);
            }
        });
    }
